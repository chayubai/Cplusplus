...
void test2()
{
	...
	//当添加了virtual关键字，此时占4个字节

	//当父类Animal添加了virtual关键字后，类的内部多出来一个指针，vfptr（virtual function pointer 虚函数表指针）
	//该指针指向虚函数表（vftable），由于是成员函数，虚函数表的内部记录了虚函数的入口地址&Animal::speak
	//当Cat类仅仅是继承Animal类，则Cat类内部结构的虚函数指针指向的虚函数表的内容也是：&Animal::speak
	//当Cat类发生父类虚函数重写，则Cat类内部结构的虚函数指针指向的虚函数表的内容变为：&Cat::speak

	//注意：
	//重写：指定是子类重写父类中的虚函数
	//1、函数返回值相同，2、函数名相同，3、形参列表相同

	//Animal* animal = new Cat;//创建Cat对象，让父类引用接收，animal本质上还是Cat对象
	//animal -> speak();//此时就会调用Cat::speak()

	//通过开发人员命令提示工具，查看没有virtual关键字的父类，此时的Cat类布局情况：为空类
	//当父类加上virtual关键字，Cat类没有speak函数，此时的Cat类的布局情况：
	//从父类继承了vfptr指针，指向vftable，入口地址0处为&Animal::speak

	//当父类加上virtual关键字，Cat类有speak函数，此时的Cat类的布局情况：
	//从父类继承了vfptr指针，指向vftable，入口地址0处为&Cat::speak
}
